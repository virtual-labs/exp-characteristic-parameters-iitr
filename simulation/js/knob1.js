var Knob2;
var container;
Knob2 = function(ram1, ui) {
   container = document.createElement('div');
  
  container.style.cssText = 'position: absolute; top:17px; left:193px; width:100px;';

  container.setAttribute('tabindex', 0);
  var ram1=document.getElementById("ram");

  ram1.parentNode.replaceChild(container, ram1);
  ram1.style.cssText = 'position: absolute; top: -10000px';
  ram1.setAttribute('tabindex', -1);
  container.appendChild(ram1);

  var settings = this.settings = this._getSettings(ram1);


  this.value = ram1.value = 5;
  this.ram1 = ram1;
...
Knob2.prototype = {
  ...
  _updateWhileMoving: function(event) {
    ...
    this.value = this.ram1.value = Math.round(value / step) * step;
    this.ui.update(percent, this.value);
    this.triggerChange();
  },

  changed: function(direction) {
    this.ram1.value = this.limit(parseFloat(this.ram1.value) + direction * (this.ram1.step || 1));
    this.value = this.ram1.value;
    this.ui.update(this._valueToPercent(), this.value);
    this.triggerChange();
  },

  update: function(value) {
    this.ram1.value = this.limit(value);
    this.value = this.ram1.value;
    this.ui.update(this._valueToPercent(), this.value);
    this.triggerChange();
  },
  ...
  triggerChange: function  () {
    if (document.createEventObject) {
      ...
      this.ram1.fireEvent("onchange", evt);
    } else {
      this.ram1.dispatchEvent(new Event('change'));
    }
  },
  ...
  _getSettings: function(ram1) {
    var labels;

    if(ram1.dataset.labels){
      labels = ram1.dataset.labels.split(',');
    }
    var settings = {
      max: labels ? labels.length-1 : parseFloat(ram1.max),
      min: labels ? 0 : parseFloat(ram1.min),
      step: parseFloat(ram1.step) || 1,
      angleoffset: 0,
      anglerange: 360,
      labels: labels
    };
    settings.range = settings.max - settings.min;
    var data = ram1.dataset;
    for (var i in data) {
